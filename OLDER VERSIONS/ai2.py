...
def evaluate_move(x, y, player, player_trail, rinzler_trail, obstacles):
    if x < 1 or x >= GRID_SIZE-1 or y < 1 or y >= GRID_SIZE-1:
        return float('inf')  # Out of bounds

    for t in player_trail + rinzler_trail + obstacles:
        if t['x'] == x and t['y'] == y:
            return float('inf')  # Collision

    # Use Manhattan distance to the player as a heuristic
...
def move_rinzler(rinzler, player, player_trail, rinzler_trail, grid_size, rinzler_direction, obstacles):
    ...
    best_move = rinzler_direction
    best_score = float('inf')

    for move, (dx, dy) in possible_moves.items():
        next_x = rinzler['x'] + dx
        next_y = rinzler['y'] + dy
        score = evaluate_move(next_x, next_y, player, player_trail, rinzler_trail, obstacles)
        if score < best_score:
            best_move = move
            best_score = score
